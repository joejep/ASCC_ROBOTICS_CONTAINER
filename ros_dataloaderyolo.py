...
from nsros.yolo_utils import COCO_NAMES
from nsros.yolo_utils import get_yolo_model, get_and_apply_mask
...
class ROSDataloader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.object_word_list = ["book", "chair"]
        self.object_index_list = [COCO_NAMES[object_type] - 1 for object_type in self.object_word_list]
    ...
    def ts_image_pose_callback(self, image: Image, pose: PoseStamped):
        ...
        if (
            ...
            masked_img = get_and_apply_mask(yolo_model, im_tensor, self.object_index_list)
            im_tensor = torch.tensor(masked_img).unsqueeze(0) / 255.0
